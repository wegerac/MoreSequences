...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(54.0, 25.0), rg.Point(-26.0, 25.0)]# All numbers are approximate

    circle = rg.Circle(rg.Point(14, 25), 40)
    ...
    # Test 3:
    expected = [rg.Point(322.0, 52.0),  # All numbers are approximate.
                rg.Point(122.0, 252.),
                rg.Point(-78.0, 52.0),
                rg.Point(122.0, -148.0)]
    circle = rg.Circle(rg.Point(122, 52), 200)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  400 slices, thin (thickness=4) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 4
    circle.fill_color = 'yellow'
    pizza(window, circle, 400, 'black', 1)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON test 3: 8 segments with thick blue lines.'
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    pointlist = generate_points_on_circle(circle, number_of_segments)
    for k in range(number_of_segments-1):
        ...
        line.attach_to(window)
        window.render()
    last = rg.Line(pointlist[0],pointlist[len(pointlist) - 1])
    last.thickness = thickness
    last.color = color
    last.attach_to(window)
    window.render()
def run_test_fancy_polygon():
    ...
    # Test  (on another window):
    title = 'FANCY POLYGON test 4:  60 blue lines on a gray circle, hops = 20.'
    window = rg.RoseWindow(600, 600, title)

    circle = rg.Circle(rg.Point(300, 300), 300)
    circle.fill_color = 'gray'
    fancy_polygon(window, circle, 60, 20, 'blue', 4)
    window.close_on_mouse_click()
